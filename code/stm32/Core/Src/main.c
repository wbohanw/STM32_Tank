...
// calibration values
int16_t accelero_pos0[3] = {0,0,0};
int16_t accelero_pos1[3] = {0,0,0};
int16_t accelero_pos2[3] = {0,0,0};
int16_t accelero_pos3[3] = {0,0,0};
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	if (GPIO_Pin == BUTTON_Pin) {
		...
		if (current_position == 0) {
			BSP_ACCELERO_AccGetXYZ(accelero_pos0);
		} else if (current_position == 1) {
			BSP_ACCELERO_AccGetXYZ(accelero_pos1);
		} else if (current_position == 2) {
			BSP_ACCELERO_AccGetXYZ(accelero_pos2);
		} else if (current_position == 3) {
			BSP_ACCELERO_AccGetXYZ(accelero_pos3);
		}
...
void calibrate_accelerometer() {
	// Compute offsets
	OFFX = (accelero_pos0[0] + accelero_pos1[0] + accelero_pos2[0] + accelero_pos3[0]) / 4;
	OFFY = (accelero_pos0[1] + accelero_pos1[1] + accelero_pos2[1] + accelero_pos3[1]) / 4;

	// Compute sensitivities
	SENSX = (accelero_pos2[0] - accelero_pos1[0]) / 2;
	SENSY = (accelero_pos0[1] - accelero_pos3[1]) / 2;

	// Compute calibration factors
	CXY = (accelero_pos0[0] - accelero_pos3[0]) / (2 * SENSX);
	CYX = (accelero_pos2[1] - accelero_pos1[1]) / (2 * SENSY);
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1)
	{
		...
		// Initial estimates
		ACCX_prime = (accelerometer[0] - OFFX) / SENSX;
		ACCY_prime = (accelerometer[1] - OFFY) / SENSY;
		...
		sprintf(txBuffer, "Calibrated Accelerometer X: %f, Y: %f, Tilt degrees: %f, Shoot: %d \r\n", ACCX, ACCY, tilt_degrees, button_status);
		button_status = 0;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if (htim->Instance == TIM2) {
		HAL_IncTick();
		if(periods<80){
			STATUS = HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_1, DAC_ALIGN_12B_R, current_tone[tone_status++]);
			if(STATUS != HAL_OK){
				Error_Handler();
			}
			if(tone_status==current_num_sample){
				tone_status=0;
				periods++;
			}
		}
